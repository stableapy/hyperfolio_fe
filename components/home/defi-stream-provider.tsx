...
export function DefiStreamProvider() {
  ...
  const { progress, startStream, stopStream } = usePositionsStream({
    addresses,
  ...
  useEffect(() => {
    ...
    const isWalletsChanged =
      walletsChangedTrigger > lastWalletsChangedTriggerRef.current;

    if (isFullSync || isWalletsChanged) {
      stopStreamRef.current();

      if (isFullSync) lastSyncTriggerRef.current = syncTrigger;
      if (isWalletsChanged)
        lastWalletsChangedTriggerRef.current = walletsChangedTrigger;
