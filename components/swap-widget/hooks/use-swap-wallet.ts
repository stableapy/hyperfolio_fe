...
export function useSwapWallet(options: UseSwapWalletOptions = {}) {
  ...
  // Combined ready state for rendering the widget
  const isWidgetReady = isConnected && isCorrectChain && isProviderReady && hasMounted && !!ethersProvider && !!ethersSigner && !!address
