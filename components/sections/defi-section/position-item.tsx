          ...
          {position.positionDetails?.token && !position.positionDetails?.pair && (
            <>
              ...
              <div className="flex items-center gap-1 min-w-0">
                ...
                <span className="font-mono text-[10px] sm:text-[11px] text-theme-cyan/70">{position.positionDetails.token.symbol}</span>
              </div>
            </>
          )}
          {/* Token icons - Pair */}
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          ...
          {isLiquidityPool && position.rewards > 0 && (
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="px-1 py-0.5 bg-theme-cyan/10 border border-theme-cyan/20 rounded flex items-center">
                    <span className="font-mono text-[9px] sm:text-[10px] text-theme-cyan tabular-nums">
                ...
                <TooltipContent className="bg-theme-bg border border-theme-border p-3">
                  <div className="font-mono text-xs space-y-1">
                    <div className="text-theme-cyan font-bold mb-2">
                      <span className="text-theme-cyan">&gt;</span> fees --uncollected
                    </div>
                    {position.positionDetails?.uncollectedFees && (
                      <>
                        ...
                        <div className="border-t border-theme-border/50 mt-2 pt-2 flex justify-between gap-4">
                          ...
                          <span className="text-theme-cyan font-bold tabular-nums">${position.positionDetails.uncollectedFees.usdValue}</span>
                        </div>
                      </>
                    )}
                  </div>
