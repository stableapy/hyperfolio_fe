...
export async function GET(request: NextRequest) {
  ...
  const aggregatedState = {
    ...
    portfolioStats: {
      ...
      totalValueWithApy: 0,
      weightedApyNumerator: 0,
      positionsWithApy: 0,
      totalPositions: 0,
      estimatedYield: { daily: 0, weekly: 0, monthly: 0 },
    },
    portfolioStatsByWallet: {} as Record<string, Record<string, unknown>>,
    completedWallets: 0,
  ...
  const streamPromise = (async () => {
    ...
    overallStreamController.signal.addEventListener('abort', async () => {
      await writeSSE({
        type: 'error',
        error:
          'Stream timeout: Loading positions took too long. Please try again.',
      });
    });
    ...
    try {
      ...
      const streamPromises = addresses.map(async (address) => {
        ...
        const abortOnTimeout = () => {
          if (
            overallStreamController.signal.aborted ||
        ...
        overallStreamController.signal.addEventListener(
          'abort',
          abortOnTimeout
        );
        ...
        try {
                ...
                try {
                  ...
                  if (data.type === 'protocol') {
                    ...
                    const protocolData = {
                      ...
                      totalValueUSDByWallet: {
                        [address]: data.data.totalValueUSD,
                      },
                      protocolStatsByWallet: {
                        [address]: data.data.protocolStats,
                      },
                    };

                    // Merge with existing protocol if already received from another wallet
                    const existingProtocol = aggregatedState.protocols.get(
                      data.data.id
                    );
                    if (existingProtocol) {
                      // Merge positions
                      const existingData = existingProtocol.protocol as Record<
                        string,
                        unknown
                      >;
                      const existingPositions = (existingData.positions ||
                        []) as unknown[];
                      const newPositions = (protocolData.positions || []).map(
                        (pos: unknown) => ({
                      ...
                      const existingStats = existingData.protocolStats as
                      ...
                      const mergedProtocol = {
                        ...
                        totalValueUSD: String(
                          parseFloat(
                            String(existingData.totalValueUSD || '0')
                          ) +
                            parseFloat(
                              String(protocolData.totalValueUSD || '0')
                            )
                        ),
                        positions: [...existingPositions, ...newPositions],
                        totalValueUSDByWallet: {
                          ...((existingData.totalValueUSDByWallet as Record<
                            string,
                            string
                          >) || {}),
                          [address]: protocolData.totalValueUSD,
                        },
                        protocolStats: mergeProtocolStats(
                          existingStats,
                          newStats
                        ),
                        protocolStatsByWallet: {
                          ...((existingData.protocolStatsByWallet as Record<
                            string,
                            unknown
                          >) || {}),
                          [address]: newStats,
                      ...
                      await writeSSE({
                        ...
                        progress: {
                          completed: aggregatedState.progress.completed,
                          total:
                            aggregatedState.progress.total * addresses.length,
                        },
                      });
                    }
                    ...
                    if (stats) {
                      aggregatedState.portfolioStats.totalValueUSD +=
                        parseFloat(stats.totalValueUSD || '0');
                      const totalValueWithApy =
                        Number(stats.totalValueWithApy) || 0;
                      const weightedApy = Number(stats.weightedApyPercent);
                      if (
                        Number.isFinite(weightedApy) &&
                        totalValueWithApy > 0
                      ) {
                        aggregatedState.portfolioStats.weightedApyNumerator +=
                          weightedApy * totalValueWithApy;
                        aggregatedState.portfolioStats.totalValueWithApy +=
                          totalValueWithApy;
                      }
                      aggregatedState.portfolioStats.positionsWithApy +=
                        stats.positionsWithApy || 0;
                      aggregatedState.portfolioStats.totalPositions +=
                        stats.totalPositions || 0;
                      aggregatedState.portfolioStatsByWallet[address] = stats;

                      const yield_ = stats.estimatedYield;
                      if (yield_) {
                        aggregatedState.portfolioStats.estimatedYield.daily +=
                          parseFloat(yield_.daily || '0');
                        aggregatedState.portfolioStats.estimatedYield.weekly +=
                          parseFloat(yield_.weekly || '0');
                        aggregatedState.portfolioStats.estimatedYield.monthly +=
                          parseFloat(yield_.monthly || '0');
                      }
                    }
          ...
          if (isTimeoutError(error) || errorMessage.includes('timeout')) {
            const timeoutMsg = getTimeoutErrorMessage(
              `positions for ${address.slice(0, 8)}...`
            );
            console.error(
              `[SSE] Timeout streaming for ${address}:`,
              timeoutMsg
            );
            ...
            console.error(
              `[SSE] Error streaming for ${address}:`,
              errorMessage
            );
      ...
      aggregatedState.portfolioStats.weightedApyPercent =
        aggregatedState.portfolioStats.totalValueWithApy > 0
          ? aggregatedState.portfolioStats.weightedApyNumerator /
            aggregatedState.portfolioStats.totalValueWithApy
          : 0;
      ...
      await writeSSE({
        ...
        portfolioStats: {
          totalValueUSD:
            aggregatedState.portfolioStats.totalValueUSD.toFixed(2),
          weightedApyPercent: aggregatedState.portfolioStats.weightedApyPercent,
          totalValueWithApy: aggregatedState.portfolioStats.totalValueWithApy,
          ...
          estimatedYield: {
            daily:
              aggregatedState.portfolioStats.estimatedYield.daily.toFixed(2),
            weekly:
              aggregatedState.portfolioStats.estimatedYield.weekly.toFixed(2),
            monthly:
              aggregatedState.portfolioStats.estimatedYield.monthly.toFixed(2),
          },
          byWallet: aggregatedState.portfolioStatsByWallet,
        },
...
function mergeProtocolStats(
  ...
  const existingYield = existing.estimatedYield as
    | Record<string, string>
    | undefined;
  const incomingYield = incoming.estimatedYield as
    | Record<string, string>
    | undefined;
  ...
  return {
    weightedApyPercent: (() => {
      const existingValue = Number(existing.totalValueWithApy) || 0;
      const incomingValue = Number(incoming.totalValueWithApy) || 0;
      const denominator = existingValue + incomingValue;
      if (denominator === 0) return null;
      return (
        (Number(existing.weightedApyPercent) * existingValue +
          Number(incoming.weightedApyPercent) * incomingValue) /
        denominator
      );
    })(),
    totalValueWithApy:
      (Number(existing.totalValueWithApy) || 0) +
      (Number(incoming.totalValueWithApy) || 0),
